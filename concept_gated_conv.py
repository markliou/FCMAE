...
def concept_gated_conv_ae():
    ...
    conv1 = tf.keras.layers.Conv2D(128, (3,3), (2,2), padding="Same", kernel_regularizer=tf.keras.regularizers.L2(1e-3), activation=mish)(conv1) # down sampling 128
    conv2 = concept_conv(conv1, 128)
    conv2 = tf.keras.layers.Conv2D(256, (3,3), (2,2), padding="Same", kernel_regularizer=tf.keras.regularizers.L2(1e-3), activation=mish)(conv2) # down sampling 64
    conv3 = concept_conv(conv2, 256)
    conv3 = tf.keras.layers.Conv2D(512, (3,3), (2,2), padding="Same", kernel_regularizer=tf.keras.regularizers.L2(1e-3), activation=mish)(conv3) # down sampling 32
    
    latent = tf.keras.layers.Conv2D(512, (1,1), (1,1), padding="Same", kernel_regularizer=tf.keras.regularizers.L2(1e-3), activation=None)(conv3) 
